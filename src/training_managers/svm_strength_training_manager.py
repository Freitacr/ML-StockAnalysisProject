...
def handle_data(ticker, training_data, out_dir, overwrite_model, combined_examples=22):
    ...
    y = y[combined_examples-1:-validation_examples]
    y_dates = y_dates[combined_examples-1:-validation_examples]
...
class SvmStrengthTrainingManager(data_provider_registry.DataConsumerBase):

    def __init__(self):
        super(SvmStrengthTrainingManager, self).__init__()
    ...
    def predict_data(self, data, passback, in_model_dir):
        model_dir = in_model_dir + path.sep + 'svm_strength_models'
...
consumer = SvmStrengthTrainingManager()
